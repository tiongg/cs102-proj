...
public class SessionRepositoryJooq implements SessionRepository {
    ...
    @Override
    public String create(String moduleSectionId, Date date, short week, Timestamp startTime, Timestamp endTimeNullable, SessionStatus status) {
    ...
    @Override
    public String create(ClassSession session) {
        return this.create(
                session.getModuleSection().getId(),
                DateUtils.toSqlDate(session.getStartTime()),
                (short) session.getWeek(),
                DateUtils.toTimestamp(session.getStartTime()),
                DateUtils.toTimestamp(session.getEndTime()),
                session.getSessionStatus()
        );
    }

    @Override
    public Optional<SessionRecord> fetchSessionById(String sessionId) {
        return dsl.select(SESSION_ID, MODULE_SECTION_ID, DATE, WEEK, START_TIME, END_TIME, STATUS, CREATED_AT)
                ...
                .fetchOptional(record -> new SessionRecord(
    ...
    @Override
    public List<SessionRecord> fetchSessionsByModuleSectionId(String moduleSectionId) {
        return dsl.selectFrom(SESSIONS_TABLE)
                .where(MODULE_SECTION_ID.eq(moduleSectionId))
                .fetch()
                .map(record -> new SessionRecord(
                        record.get(SESSION_ID),
                        record.get(MODULE_SECTION_ID),
                        record.get(DATE),
                        record.get(WEEK),
    ...
    @Override
    public List<SessionRecord> fetchSessionsByTeacherUserIdAndWeek(String teacherUserId, short week) {
